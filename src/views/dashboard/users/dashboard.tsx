					...
					{courses.map((course) => {
						...
						return (
							...
							>
								{hasEnrolled ? (
									<form
										hx-get={`/dashboard/my-courses/${course.id}/first-lesson`}
										class="block"
									>
									...
									<form
										hx-post={`/dashboard/courses/${course.id}/buy`}
										class="block"
									>
										<input name="amount" hidden value={String(course.price)} />
										<button class="text-sm btn-outline">
											Buy for IDR {currencyFormat.format(course.price)}
										</button>
									</form>
								)}
							</CourseCard>
